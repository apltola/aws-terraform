...
resource "aws_subnet" "private_a" {
  ...
  vpc_id            = aws_vpc.main.id
  availability_zone = "${data.aws_region.current.name}a" # eu-north-1a
...
resource "aws_route" "private_a_internet_out" {
  route_table_id         = aws_route_table.private_a.id
  nat_gateway_id         = aws_nat_gateway.public_a.id # because private subnet doesn't have any ip addresses associated with it, we have to go through our public subnet nat gateway
  destination_cidr_block = "0.0.0.0/0"
}
...
resource "aws_subnet" "private_b" {
  ...
  vpc_id            = aws_vpc.main.id
  availability_zone = "${data.aws_region.current.name}b" # eu-north-1b
